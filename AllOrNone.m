...
%%----------- Runs HH Model to Find Pulses and Conductances  ----------------

[t,y] = RunHH(20,100,0.1);  % run the HH model for 20ms applying a currrent pulse 100uA/cm^2 high & 0.1ms wide.
...
for k = 1:N,
    [t,y] = RunHH(10,current_amplitude_list(k),0.1);
...
vmax = zeros(N,1);
for k = 1:N,
  [t,y] = RunHHl(10, current_amplitude_list(k), 0.05);
  v = y(:,1);
  vmax(k) = max(v);
end
figure;
